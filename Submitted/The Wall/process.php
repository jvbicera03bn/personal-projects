...
/* $_POST['form_type'] = 'Log In'; */ // FOR DEBUG ONLY
...
if ($_POST['form_type'] === 'register') {
    ...
    foreach ($_POST as $key => $value) {
        if (empty($value)) {
            $_SESSION['registerErr'][] = "Hey you forgot to fill this field: ". ucfirst(str_replace('_', ' ', $key));
            $fieldsok = true;
        } else {
            $fieldsok === false;
...
if ($_POST['form_type'] === 'login') {
    ...
    if (count($user) > 0) {
        ...
        if ($user[0]['password'] == $encrypted_password) {
            ...
            $_SESSION['personal_infos'] = fetch_record($query);
