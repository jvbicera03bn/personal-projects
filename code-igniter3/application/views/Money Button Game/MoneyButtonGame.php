...
<head>
    ...
    <link rel="stylesheet" href="<?= base_url('application/assets/Moneybttngame.css'); ?>" type="text/css">
...
<body>
    <header>
        <div>
            <h2>Your Money <?= $this->session->userdata('money') ?> </h2>
            <p>Chance Left <?= $this->session->userdata('chance') ?>
            <p>
        </div>
        <form method="post" action="moneybuttongame">
    ...
    <main>
        ...
        <div id="gmhost">
            ...
            <?php if (empty($this->session->userdata('bet'))) {
                foreach ($this->session->userdata['history'] as $history) {
                    ...
                    <?php } ?>
                <?php }
                if ($this->session->userdata('no_chance') === 0) { ?>
                    <p>Game Over</p>
                <?php } ?>
