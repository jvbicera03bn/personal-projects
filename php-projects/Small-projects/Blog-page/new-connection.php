...
define('DB_PORT', '3306');
define('DB_USER', 'root');
define('DB_PASS', '1232'); //may need to set DB_PASS as 'root'
define('DB_DATABASE', 'encryption'); //make sure to set your database
...
function review(){
  $query = "SELECT reviews.review_id,user.first_name, user.last_name,reviews.content, DATE_FORMAT(reviews.created_at, '%b %D %Y %h:%i%p' ) as date  FROM reviews
    INNER JOIN user ON reviews.user_id = user.user_id ORDER BY date DESC;";
...
INNER JOIN user ON reply.user_id = user.user_id INNER JOIN reviews ON reply.review_id = reviews.review_id ORDER BY date DESC;";
